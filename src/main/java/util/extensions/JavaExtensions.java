...
public class JavaExtensions {
   ...
   public static String asAttr(Map attributes, Object condition) {
      if (eval(condition)) {
   ...
   public static String asAttr(Map attributes) {
   ...
   public static String format(Number number, String pattern) {
      DecimalFormatSymbols symbols = new DecimalFormatSymbols(); // TODO: new Locale(Lang.get())
      return new DecimalFormat(pattern, symbols).format(number);
   }

   public static String format(Date date) {
      // Get the pattern from the configuration
      return new SimpleDateFormat().format(date); // TODO: I18N.getDateFormat()
   }
   ...
   */
   ...
   public static String urlEncode(String entity) {
      try {
         return URLEncoder.encode(entity, "utf-8"); //TODO: Http.Response.current().encoding
      } catch (UnsupportedEncodingException e) {
         Logger.getLogger(JavaExtensions.class.getName()).log(Level.SEVERE, e.getMessage());
      }
      return entity;
   }
   ...
   public static String formatCurrency(Number number, String currencyCode) {
      ...
      NumberFormat numberFormat = NumberFormat.getCurrencyInstance(); // TODO: new Locale(Lang.get())
      numberFormat.setCurrency(currency);
      numberFormat.setMaximumFractionDigits(currency.getDefaultFractionDigits());
      String s = numberFormat.format(number);
      // TODO: s = s.replace(currencyCode, I18N.getCurrencySymbol(currencyCode));
      return s;
   }
